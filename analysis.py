...
class Freq:
    def frequency(country,filter):
        ...
        outputFile.write(str("%.2f" % Ppercent) + " " + str("%.2f" % Npercent) + " " + str("%.2f" % NePercent) + "\n")

        x_axis = ["Stop Words", "Neutral Words", "Positive Words", "Negative words"]
        y_axis = [StWordPercent, NeutWordPercent, PosWordPercent, NegWordPercent]
        #df = pd.DataFrame(dict(x_axis = ["Stop Words", "Neutral Words", "Positive Words", "Negative words"], y_axis = [StWordPercent, NeutWordPercent, PosWordPercent, NegWordPercent]))
...
line=["canada.txt","indonesia.txt","korea.txt","singapore.txt","us.txt"]
for x in filter:
    f = open(x, 'r+')
    f.truncate(0)
...
with open('output.txt','r') as data:
    ...
    for line in data:
        column = line.split(' ')
        positiveW.append(float(column[0]))
        negativeW.append(float(column[1]))
        #neutralW.append(float(column[2]))

    #plot graph
    Country = ["Canada", "Indonesia", "Korea", "Singapore", "Us"]
    ...
    print("Based on the result, it can be concluded that article about Indonesia"
